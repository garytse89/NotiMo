...
class MainWindow(QtGui.QMainWindow):
    ...
    def connectedQuery(self,sock):
        try:
            ...
            self.sock.send('B')
            self.sock.send('C')
            self.main_widget.btStatus('Bluetooth is ON')
        except:
            self.main_widget.btStatus('Try connecting again, takes a few tries.')
    
    def update(self):
        print "Turn light green"
        self.sock.send('G') # IF BLUETOOTH ISN'T ON, ERROR MESSAGES WILL SHOW IN CONSOLE


    def clear(self):
        print "Turn off LED"
        self.sock.send('C') # IF BLUETOOTH ISN'T ON, ERROR MESSAGES WILL SHOW IN CONSOLE
...
class MainWidget(QtGui.QWidget):
    ...
    def initUI(self):
        ...
        self.btText = QtGui.QLabel('Bluetooth is OFF.\nConnect to Bluetooth by pressing the icon above.', self)
        ...
        gAutoCheck.clicked.connect(self.gmailCheck)
        ...
        # 2. generate layout for the tab
        dLayout = QtGui.QGridLayout(debugTab)
        # 3. generate buttons for the tab
        redLEDBtn = QtGui.QRadioButton('Test red LED')
        greenLEDBtn = QtGui.QRadioButton('Test green LED')
        blueLEDBtn = QtGui.QRadioButton('Test blue LED')
        # 4. add buttons to the layout
        dLayout.addWidget(redLEDBtn, 1, 0)
        dLayout.addWidget(greenLEDBtn, 1, 1)
        dLayout.addWidget(blueLEDBtn, 1, 2)
        ...
        #
        tab_widget.addTab(debugTab, "Test LEDs")
